...
class PatchMerging2D_word(nn.Module):
    ...
    def forward(self, x, H_out, W_out, H_in, W_in):
        ...
        x = torch.cat(
            [torch.cat([x0, x1], 3), torch.cat([x2, x3], 3)], 4
        )  # B, H/2, W/2, 2*H_in, 2*W_in, C
        x = x.reshape(-1, 2 * H_in, 2 * W_in, C).permute(
            0, 3, 1, 2
        )  # B_N/4, C, 2*H_in, 2*W_in
...
class Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        H_out, W_out = (
            H // 8,
            W // 8,
        )  # Each visual sentence corresponds to 8x8 pixel area of the original image
        H_in, W_in = (
            4,
            4,
        )  # Every visual sentence is composed of 4x4 visual words, Every visual word at the stem stage corresponds to 2x2 pixel area of the original image
        # inner_tokens
        ...
        inner_tokens = inner_tokens.reshape(
            B * H_out * W_out, self.inner_dim, H_in * W_in
        ).transpose(
            1, 2
        )  # B*N, C, 4*4
        # outer_tokens
...
class Stage(nn.Module):
    ...
    def __init__(
        ...
        self.relative_pos = nn.Parameter(
            torch.randn(1, outer_head, num_patches, num_patches // sr_ratio // sr_ratio)
        )

    def forward(self, inner_tokens, outer_tokens, H_out, W_out, H_in, W_in):
        for blk in self.blocks:
            inner_tokens, outer_tokens = blk(
                inner_tokens, outer_tokens, H_out, W_out, H_in, W_in, self.relative_pos
            )
...
class PyramidMiM_enc(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            mid_out = outer_tokens.reshape(
                b, int(math.sqrt(l)), int(math.sqrt(l)), m
            ).permute(0, 3, 1, 2)
...
class MiM_plus(BaseModel):
    def __init__(self, layer_blocks, channels, in_channels=3, img_size=256, loss=BinaryDiceLoss):
        super(MiM_plus, self).__init__()
        self.mim_backbone = PyramidMiM_enc(in_chans=in_channels, img_size=img_size)
        ...
        self.finaldeconv = PatchExpand2D(channels[1])
        ...
        self.final_up = nn.Sequential(
            nn.ConvTranspose2d(channels[1], channels[1], 4, 2, 1),
            nn.Conv2d(channels[1], channels[1], 3, 1, 1),
            nn.BatchNorm2d(channels[1]),
            nn.GELU(),
        )
        self.final_wideview = nn.Sequential(
            nn.Conv2d(channels[1], channels[1], 7, 1, 3),
            nn.BatchNorm2d(channels[1]),
            nn.GELU(),
        )
    ...
    def forward(
        self, x, gt=None, mode="predict"
    ):  # the input is of size (b,3,512,512), the output is of size (b,1,512,512), where the num_class=1 in ISTD.
        if x.shape[1] == 1:
            x = einops.repeat(x, "b c w h -> b (repeat c) w h", repeat=3)
        _, _, hei, wid = x.shape
        ...
        upc1 = self.final_up(upc1)
        final_res = self.final_wideview(upc1)
        upc1 = self.final_proj(final_res + upc1)
        pred = self.head(upc1)
        out = torch.sigmoid(F.interpolate(pred, size=[hei, wid], mode="bilinear"))

        if mode == "predict":
            return out
        else:
            return {"loss": self.loss(out, gt) * 10000}
